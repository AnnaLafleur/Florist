document.addEventListener('DOMContentLoaded', () => {
  ...
  function showContent(step) {
    ...
    // Диспатчим событие transitionEnd с указанием текущего шага
    const activeStep = document.querySelector('.step.active');
    const stepClass = activeStep ? activeStep.className.match(/step-\d+/)?.[0] : '';
    document.dispatchEvent(new CustomEvent('transitionEnd', { detail: { step: stepClass } }));
  }

  // Функция для анимации слева направо (для "Продолжить")
  function animateLeftToRight(e, href) {
    e.preventDefault();
  ...
  function animateRightToLeft(e, href) {
    e.preventDefault();
    document.dispatchEvent(new CustomEvent('transitionStart'));
    hideContent();
