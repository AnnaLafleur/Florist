...
const itemsGreen = [
    { key: 'g1', text: 'eucalyptus', isActive: false },
    { key: 'g2', text: 'gypsophila', isActive: false },
    { key: 'g3', text: 'eucalyptus+bud', isActive: false },
    { key: 'g4', text: 'fern+bud', isActive: false }
];
...
const state = {
    ...
    step2: {
        container: document.querySelector('.step-2 .directory-grid[data-type="flower"]'),
        items: itemsFlower,
        order: itemsFlower.map((_, i) => i),
        isAnimating: false,
        lastGridCols: null,
    ...
    step2Green: {
        container: document.querySelector('.step-2 .directory-grid[data-type="green"]'),
        items: itemsGreen,
        order: itemsGreen.map((_, i) => i),
        isAnimating: false,
        lastGridCols: null,
        lastPos: null,
        savedRightSectionWidth: null
...
function triggerBackgroundWidthUpdate(step) {
    if (isCreatorPage && typeof updateBackgroundWidth === 'function') {
        const rightSection = step !== 'directory' ? document.querySelector(`.step-${step === 'prints' ? 3 : step === 'step2Green' ? 2 : step} .right-section`) : null;
        const stateKey = step;

        // На ПК версии не применяем сохраненную ширину - она может быть с мобильной
...
function saveStateToLocalStorage(step) {
    ...
    const stateKey = step === 'step3' ? 'gridStateStep3Texture' : step === 'prints' ? 'gridStateStep3Print' : step === 'step2Green' ? 'gridStateStep2Green' : 'gridState';
...
// Мобильный вариант: column-wrap flex, 2 ряда, горизонтальный скролл.
// Позиция элемента с индексом pos:
//   col = Math.floor(pos / MOBILE_ROWS)  — номер колонки (горизонталь)
//   row = pos % MOBILE_ROWS              — номер ряда (вертикаль)
// Это полный аналог десктопной раскладки, повёрнутый на 90°:
//   десктоп: pos → row = pos/cols, col = pos%cols  → left=col*(W+gapX), top=row*(H+gapY)
//   мобайл:  pos → col = pos/ROWS,  row = pos%ROWS → left=col*(W+gapX), top=row*(H+gapY)
const MOBILE_ROWS = 2;
const MOBILE_ITEM_W = 240;
const MOBILE_ITEM_H = 230;
const MOBILE_COL_GAP = 10;
const MOBILE_ROW_GAP = 10;
const MOBILE_PAD_LEFT = 12;
const MOBILE_PAD_TOP  = 4;

function getMobileXY(pos) {
    const col = Math.floor(pos / MOBILE_ROWS);
    const row = pos % MOBILE_ROWS;
    return {
        x: MOBILE_PAD_LEFT + col * (MOBILE_ITEM_W + MOBILE_COL_GAP),
        y: MOBILE_PAD_TOP  + row * (MOBILE_ITEM_H + MOBILE_ROW_GAP)
    };
}

function getMobilePosFromXY(x, y, totalItems) {
    const col = Math.max(0, Math.round((x - MOBILE_PAD_LEFT) / (MOBILE_ITEM_W + MOBILE_COL_GAP)));
    const row = Math.max(0, Math.min(MOBILE_ROWS - 1, Math.round((y - MOBILE_PAD_TOP) / (MOBILE_ITEM_H + MOBILE_ROW_GAP))));
    const totalCols = Math.ceil(totalItems / MOBILE_ROWS);
    return Math.min(totalItems - 1, Math.max(0, Math.min(totalCols - 1, col) * MOBILE_ROWS + row));
}

function isMobileStep2(step) {
    return window.innerWidth <= 600 && (step === 'step2' || step === 'step2Green');
}
...
function moveToFirst(index, grid, itemEl, step) {
    ...
    const rightSection = isCreatorPage && step !== 'directory' ? document.querySelector(`.step-${step === 'prints' ? 3 : step === 'step2Green' ? 2 : step} .right-section`) : null;
    if (rightSection) {
        state[stateKey].savedRightSectionWidth = rightSection.style.width || getComputedStyle(rightSection).width;
    }

    const mobile = isMobileStep2(step);
    if (!mobile) grid.style.display = 'block';
    ...
    if (currentPos === 0) {
        ...
        if (!mobile) grid.style.display = 'grid';
    ...
    if (mobile) {
        const startXY = getMobileXY(currentPos);
        startX = startXY.x; startY = startXY.y;
        const endXY = getMobileXY(0);
        endX = endXY.x; endY = endXY.y;
        const maxDist = Math.sqrt(
            Math.pow((Math.ceil(state[step].items.length / MOBILE_ROWS) - 1) * (MOBILE_ITEM_W + MOBILE_COL_GAP), 2) +
            Math.pow((MOBILE_ROWS - 1) * (MOBILE_ITEM_H + MOBILE_ROW_GAP), 2)
        );
        const dist = Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2));
        duration = BASE_ANIMATION_DURATION + (maxDist > 0 ? dist / maxDist : 0) * 600;
    } else {
        const startRow = Math.floor(currentPos / cols);
        const startCol = currentPos % cols;
        startX = startCol * (ITEM_WIDTH + COLUMN_GAP) + PADDING_LEFT;
        startY = startRow * (ITEM_HEIGHT + ROW_GAP) + PADDING_TOP;
        endX = PADDING_LEFT; endY = PADDING_TOP;
        duration = step === 'step2Green' ? 500 : BASE_ANIMATION_DURATION + (Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2)) / Math.sqrt(Math.pow((cols - 1) * (ITEM_WIDTH + COLUMN_GAP), 2) + Math.pow((rows - 1) * (ITEM_HEIGHT + ROW_GAP), 2))) * 600;
    }
    ...
    animateItem(
        ...
        (currentX, currentY) => {
            const newPos = mobile
                ? getMobilePosFromXY(currentX, currentY, state[step].items.length)
                : (() => {
...
function returnToPosition(index, grid, itemEl, step) {
    ...
    if (mobile) {
        const startXY = getMobileXY(currentPos);
        const endXY = getMobileXY(targetPos);
        startX = startXY.x; startY = startXY.y;
        endX = endXY.x; endY = endXY.y;
        const maxDist = Math.sqrt(Math.pow((Math.ceil(state[step].items.length / MOBILE_ROWS) - 1) * (MOBILE_ITEM_W + MOBILE_COL_GAP), 2) + Math.pow((MOBILE_ROWS - 1) * (MOBILE_ITEM_H + MOBILE_ROW_GAP), 2));
        const dist = Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2));
        duration = maxDist > 0 ? BASE_ANIMATION_DURATION + (dist / maxDist) * 600 : 400;
    } else {
        const startRow = Math.floor(currentPos / cols);
        const startCol = currentPos % cols;
        startX = startCol * (ITEM_WIDTH + COLUMN_GAP) + PADDING_LEFT;
        ...
        endX = endCol * (ITEM_WIDTH + COLUMN_GAP) + PADDING_LEFT;
        endY = endRow * (ITEM_HEIGHT + ROW_GAP) + PADDING_TOP;
        duration = step === 'step2Green' ? 400 : BASE_ANIMATION_DURATION + (Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2)) / Math.sqrt(Math.pow((cols - 1) * (ITEM_WIDTH + COLUMN_GAP), 2) + Math.pow((rows - 1) * (ITEM_HEIGHT + ROW_GAP), 2))) * 600;
    }
...
function updatePositions(step, excludeIndex = null) {
    ...
    if (!isCreatorPage || step === 'directory') {
    ...
    } else if (step === 'step2' || step === 'step2Green') {
        if (window.innerWidth <= 600) {
            const totalItems = state[step].items.length;
            const totalCols = Math.ceil(totalItems / MOBILE_ROWS);
            const containerW = MOBILE_PAD_LEFT + totalCols * (MOBILE_ITEM_W + MOBILE_COL_GAP) - MOBILE_COL_GAP + MOBILE_PAD_LEFT;
            const containerH = MOBILE_PAD_TOP + MOBILE_ROWS * (MOBILE_ITEM_H + MOBILE_ROW_GAP) - MOBILE_ROW_GAP + MOBILE_PAD_TOP;
            ...
            container.style.scrollbarGutter = '';
            container.style.boxSizing = 'border-box';

            // Ставим все ячейки в absolute по формуле column-wrap
            const allItems = Array.from(container.querySelectorAll('.directory-item:not(.animating)'));
            allItems.forEach(itemEl => {
                const idx = parseInt(itemEl.dataset.index, 10);
                const pos = state[step].order.indexOf(idx);
                const { x, y } = getMobileXY(pos);
                itemEl.style.position = 'absolute';
                itemEl.style.left = `${x}px`;
                itemEl.style.top = `${y}px`;
                itemEl.style.width = `${MOBILE_ITEM_W}px`;
                itemEl.style.height = `${MOBILE_ITEM_H}px`;
                itemEl.style.flexShrink = '';
                itemEl.style.scrollSnapAlign = '';
        ...
        container.style.width = '100%';
        container.style.height = 'calc(100% - 280px)';
        ...
        container.style.columnGap = `${COLUMN_GAP}px`;
        container.style.rowGap = `${ROW_GAP}px`;
        container.style.paddingLeft = step === 'step2' ? '30px' : '20px';
...
document.addEventListener('DOMContentLoaded', () => {
    ['directory', 'step2', 'step2Green', 'step3', 'prints'].forEach(step => {
...
document.querySelectorAll('.directory-item .hackyBox').forEach(checkbox => {
    checkbox.addEventListener('click', (event) => {
        ...
        const step = checkbox.closest('.step-2 .directory-grid[data-type="flower"]') ? 'step2' :
                    checkbox.closest('.step-2 .directory-grid[data-type="green"]') ? 'step2Green' :
                    checkbox.closest('.step-3 .directory-grid[data-type="print"]') ? 'prints' :
                    checkbox.closest('.step-3') ? 'step3' : 'directory';
        const stateKey = step;
        if (state[stateKey].isAnimating) return;
        ...
        if (checkbox.checked) {
            ...
            setTimeout(() => moveToFirst(index, grid, directoryItem, step), 550);
        } else {
