...
function getGridCols() {
    ...
    console.log(`Window width: ${window.innerWidth}px, Available width: ${containerWidth}px, Calculated columns: ${cols}`);
    return cols;
}
...
function returnToAlphabeticalPosition(index, grid, itemEl) {
    ...
    // Обновляем порядок и DOM до анимации
    const newOrder = [...order];
    newOrder.splice(currentPos, 1);
...
document.querySelectorAll('.hackyBox').forEach(checkbox => {
    checkbox.addEventListener('click', (event) => {
        ...
        if (checkbox.checked) {
            ...
            returnToAlphabeticalPosition(index, grid, directoryItem); // Убрано setTimeout
        }
        saveStateToLocalStorage(); // Сохраняем состояние после изменения
    });
});
