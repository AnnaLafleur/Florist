...
const state = {
    directory: {
        container: document.querySelector('.directory-grid:not(.step-2 .directory-grid):not(.stepAngryBirds3 .directory-grid)'),
        items: items,
    ...
    step3: {
        container: document.querySelector('.step-3 .right-section .directory-grid'),
        items: itemsStep3,
        order: itemsStep3.map((_, i) => i),
        isAnimating: false,
        lastGridCols: null,
        lastPos: null,
        savedRightSectionWidth: null
...
function triggerBackgroundWidthUpdate(step) {
    if (isCreatorPage && typeof updateBackgroundWidth === 'function') {
        const rightSection = step !== 'directory' ? document.querySelector(`.step-${step} .right-section`) : null;
        const stateKey = step;
...
function saveStateToLocalStorage(step) {
    ...
    const stateKey = step === 'step3' ? 'gridStateStep3' : 'gridState';
...
function moveToFirst(index, grid, itemEl, step) {
    ...
    const rightSection = isCreatorPage && step !== 'directory' ? document.querySelector(`.step-${step} .right-section`) : null;
    if (rightSection) {
...
function returnToPosition(index, grid, itemEl, step) {
    ...
    const rows = getGridRows(step);
    const currentPos = state[stateKey].order.indexOf(index);
    const targetPos = step === 'step3' ? findNumericalPosition(index, step) : findAlphabeticalPosition(index, step);
...
document.addEventListener('DOMContentLoaded', () => {
    ['directory', 'step2', 'step3'].forEach(step => {
        ...
        if (grid) {
            ...
            if (step === 'directory') {
                ...
                state[step].items.forEach((item, i) => {
                    item.isActive = state.directory.items[i].isActive;
                });
            } else {
                restoreStateFromLocalStorage(step);
            }
...
document.querySelectorAll('.directory-item .hackyBox').forEach(checkbox => {
    checkbox.addEventListener('click', (event) => {
        ...
        const step = checkbox.closest('.step-2') ? 'step2' : checkbox.closest('.step-3') ? 'step3' : 'directory';
        const stateKey = step;
        if (state[stateKey].isAnimating) return;
