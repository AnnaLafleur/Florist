...
const RIGHT_SECTION_PADDING = isCreatorPage ? 85 : 0; // padding-right: 30px (.right-section) + 25px (.directory-grid) + padding-left: 30px
...
function updateOrder() {
    ...
    console.log('Initial order:', order.map(i => items[i].text));
}
...
function calculateContainerDimensions(cols) {
    return {
        ...
        height: (ITEM_HEIGHT * getGridRows()) + (ROW_GAP * (getGridRows() - 1)) + (isCreatorPage ? PADDING_TOP : 0)
    };
}
