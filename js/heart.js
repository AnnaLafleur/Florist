...
function moveToFirst(index, grid, itemEl, step) {
    ...
    const duration = step === 'step2Green' ? 500 : BASE_ANIMATION_DURATION + (Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2)) / Math.sqrt(Math.pow((cols - 1) * (ITEM_WIDTH + COLUMN_GAP), 2) + Math.pow((rows - 1) * (ITEM_HEIGHT + ROW_GAP), 2))) * 600;
...
function returnToPosition(index, grid, itemEl, step) {
    ...
    const duration = step === 'step2Green' ? 400 : BASE_ANIMATION_DURATION + (Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2)) / Math.sqrt(Math.pow((cols - 1) * (ITEM_WIDTH + COLUMN_GAP), 2) + Math.pow((rows - 1) * (ITEM_HEIGHT + ROW_GAP), 2))) * 600;
