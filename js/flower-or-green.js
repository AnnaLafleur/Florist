document.addEventListener('DOMContentLoaded', function() {
    ...
    let activeTab = 'flower'; // 'flower' или 'green'
    ...
    function handleItemClick(event, grid, type) {
        ...
        // Снимаем предыдущее выделение (как на 3-м этапе — снять нельзя, только перенести)
    ...
    function fixGridPositions() {
        ...
        if (isMobile) {
            // На мобильном: панель снизу 42vh, grid горизонтальный
            if (rightSection) {
                rightSection.style.top = '';
                rightSection.style.paddingTop = '';
                rightSection.style.height = '42vh';
                rightSection.style.minHeight = '';
                rightSection.style.position = 'relative';
            }
            const applyMobileGrid = (grid) => {
                if (!grid || grid.classList.contains('grid--hidden')) return;
                ...
                // Уменьшаем ячейки через scale на каждой
            ...
            applyMobileGrid(flowerGrid);
            applyMobileGrid(greenGrid);

            // Динамический зум под высоту right-section (2 строки)
            if (window.updateMobileCellZoom) {
                setTimeout(() => window.updateMobileCellZoom(), 50);
            }
            return;
        }

        // Десктоп — оригинальная логика
        if (rightSection) {
        ...
        if (flowerGrid && !flowerGrid.classList.contains('grid--hidden')) {
            flowerGrid.style.marginTop = '20px';
            flowerGrid.style.paddingTop = '0';
            flowerGrid.style.paddingBottom = '20px';
            flowerGrid.style.height = 'auto';
            flowerGrid.style.minHeight = 'calc(100vh - 245px)';
            flowerGrid.style.rowGap = '15px';
            flowerGrid.style.paddingLeft = '30px';
            flowerGrid.style.paddingRight = '25px';
            flowerGrid.style.columnGap = '15px';
        }

        if (greenGrid && !greenGrid.classList.contains('grid--hidden')) {
            greenGrid.style.marginTop = '20px';
            greenGrid.style.paddingTop = '0';
            greenGrid.style.paddingBottom = '20px';
            greenGrid.style.height = 'auto';
            greenGrid.style.minHeight = 'calc(100vh - 245px)';
            greenGrid.style.rowGap = '15px';
    ...
    function adjustContainerHeight() {
        // На мобильном не трогаем высоту — управляет CSS
        if (window.innerWidth <= 600) return;

        const rightSection = document.querySelector('.step-2 .right-section');
        const activeGrid = activeTab === 'flower' ? flowerGrid : greenGrid;
    ...
    function toggleGrids(showFlower, isInitial = false) {
        ...
        activeTab = showFlower ? 'flower' : 'green';
        ...
        requestAnimationFrame(() => {
            if (showFlower) {
                flowerGrid.classList.remove('grid--hidden');
                greenGrid.classList.add('grid--hidden');
            } else {
                greenGrid.classList.remove('grid--hidden');
                flowerGrid.classList.add('grid--hidden');
            }
    ...
    style.textContent = `
        /* Скрытие гридов — использует !important чтобы не перебивалось другими стилями */
        .step-2 .right-section .directory-grid.grid--hidden {
    ...
    // Инициализация выделения первых ячеек по умолчанию
    function initDefaultSelection() {
        // Flower: выделяем первую ячейку (carnation)
        if (!step2State.lastSelectedFlower) {
            const firstFlowerItem = flowerGrid.querySelector('.directory-item');
            if (firstFlowerItem) {
                const flowerName = firstFlowerItem.dataset.flowerName;
                const radio = firstFlowerItem.querySelector('.item-content__input');
                const content = firstFlowerItem.querySelector('.item-content');
                if (flowerName && radio && content) {
                    radio.checked = true;
                    content.classList.add('selected');
                    step2State.lastSelectedFlower = flowerName;
                }
            }
        }

        // Green: выделяем первую ячейку (asparagus)
        if (!step2State.lastSelectedGreen) {
            const firstGreenItem = greenGrid.querySelector('.directory-item');
            if (firstGreenItem) {
                const greenName = firstGreenItem.dataset.greenName;
                const radio = firstGreenItem.querySelector('.item-content__input');
                const content = firstGreenItem.querySelector('.item-content');
                if (greenName && radio && content) {
                    radio.checked = true;
                    content.classList.add('selected');
                    step2State.lastSelectedGreen = greenName;
                }
            }
        }

        // Показываем иконки цвета для активной вкладки flower
        if (flowerRadio.checked && window.showColorIconsForSelected) {
            setTimeout(() => window.showColorIconsForSelected(), 150);
        }
    }
