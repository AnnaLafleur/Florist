class FlowerAnimation {
    ...
    // Добавляем метод для предварительной загрузки изображений
            ...
            flowers.flatMap(flower =>
                colors.map(color => {
                    ...
                    if (!FlowerAnimation.spriteCache[spriteKey]) {
                        ...
                        FlowerAnimation.spriteCache[spriteKey].src = `img/animation/${flower}/${color}.png`;
                    }
                    if (!FlowerAnimation.spriteCache[firstFrameKey]) {
                        FlowerAnimation.spriteCache[firstFrameKey] = new Image();
                        FlowerAnimation.spriteCache[firstFrameKey].src = `img/animation/${flower}/${color}/0001.png`;
                    }
                    return [
                        new Promise(resolve => {
