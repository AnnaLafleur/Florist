class FlowerAnimation {
    ...
    constructor(flowerName, elementId) {
        ...
        if (this.img) {
            ...
            this.img.style.opacity = '1'; // Ensure no fading
        }
    }
    ...
    playAnimation(frameDelay = 40, parentNode = null, targetImg = null, width = null, height = null) {
        ...
        const createCanvas = () => {
            ...
            this.canvas.style.opacity = '1'; // Ensure no fading

            if (parentNode && targetImg) {
                // Для внешних вызовов (модальное окно)
                const fallbackSize = Math.max(window.innerWidth * 0.9, 200); // Ensure min 200px
                ...
                // Для стандартных случаев (ячейки)
            ...
            // Debugging: Log canvas and parent details
            const parentRect = insertParent.getBoundingClientRect();
            console.log(`Canvas created for ${this.flowerName} (${this.elementId || 'modal'}): ` +
                `width=${this.canvas.width}, height=${this.canvas.height}, ` +
                `style=${this.canvas.style.cssText}, ` +
                `parentRect={top:${parentRect.top}, left:${parentRect.left}, width:${parentRect.width}, height:${parentRect.height}}`);
        };
        ...
        const startAnimation = () => {
            ...
            // Debugging: Log drawing parameters
            console.log(`Drawing sprite for ${this.flowerName} (${this.elementId || 'modal'}): ` +
                `scale=${scale}, drawWidth=${drawWidth}, drawHeight=${drawHeight}, ` +
                `offsetX=${offsetX}, offsetY=${offsetY}, spriteWidth=${this.spriteImg.width}, spriteHeight=${this.spriteImg.height}`);
        ...
        if (!this.spriteImg.complete) {
            ...
            tempImg.onload = () => {
                ...
                // Debugging: Log temporary image drawing
                console.log(`Drawing temp image for ${this.flowerName} (${this.elementId || 'modal'}): ` +
                    `scale=${scale}, drawWidth=${drawWidth}, offsetX=${offsetX}`);
            ...
            tempImg.onerror = () => {
                console.error(`Failed to load temp image: ${this.firstFrame}`);
                this.finishAnimation(insertTarget);
            };
        } else {
            startAnimation();
    ...
    finishAnimation(insertTarget) {
        ...
        if (insertTarget) {
            ...
            insertTarget.style.opacity = '1'; // Ensure no fading
        ...
        // Debugging: Log animation completion
        console.log(`Animation finished for ${this.flowerName} (${this.elementId || 'modal'})`);
    }
}
