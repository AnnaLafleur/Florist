document.addEventListener('DOMContentLoaded', function() {
    ...
    // 1. Function to hide color icons immediately
    ...
    // 2. Functions for handling color icons
    function updateBackgroundWidth() {
        // Only run for step 2 or step 3
        if (!document.querySelector('.step-2.active, .step-3.active')) {
            // Reset right-section styles for step 1 to prevent shifts
        ...
        if (isStep3Print) {
            ...
            const scrollbarWidth = 20; // Fixed scrollbar width for step 3
            ...
            rightSection.style.width = `${Math.ceil(totalWidth)}px`; // Use Math.ceil for pixel-perfect width
            rightSection.style.minWidth = `${Math.ceil(totalWidth)}px`; // Enforce min-width for stability
            grid.style.gridTemplateColumns = `repeat(${columns}, ${cellWidth}px)`; // Set explicit columns
        } else {
            // Step 2 logic
    ...
    function updateColorIconsPosition() {
        ...
        const containerWidth = 60; // Consistent width for both steps
        if (isStep3) {
            ...
            if (selectedShape === 'form-square' || selectedShape === 'form-rectangle') {
                colorIconContainer.style.left = `${Math.floor(rightEdge - containerWidth + 20)}px`; // Pixel-perfect positioning
            } else {
                colorIconContainer.style.left = `${Math.floor(rightEdge - containerWidth)}px`; // Pixel-perfect positioning
            }
        } else {
            colorIconContainer.style.left = `${Math.floor(rightEdge - containerWidth)}px`; // Pixel-perfect positioning
        }
    ...
    function createColorIconContainer() {
        ...
        style.textContent = `
            .color-icon-container {
                ...
                z-index: 0; /* Set to layer 0 to stay below flower cells */
                overflow: visible;
                ...
                visibility: hidden;
                opacity: 0;
                min-width: 60px; /* Consistent width for both steps */
                pointer-events: none;
            }
            .color-icons-wrapper {
                display: flex;
                ...
                min-height: 100%;
                pointer-events: auto;
                z-index: 1; /* Slightly above container for internal stacking */
            }
            .color-icon {
                position: relative;
                opacity: 0;
                ...
                pointer-events: auto;
                cursor: pointer;
                z-index: 2; /* Above wrapper for interaction */
            }
            .color-icon.disabled {
                pointer-events: none;
                cursor: not-allowed;
            ...
            .color-icon:not(.disabled):hover {
                transform: scale(1.1); /* Visible hover effect */
                z-index: 3; /* Ensure hover is above other icons */
            }
            ...
            .step-2.active .right-section, .step-3.active .right-section {
                box-sizing: border-box;
                padding-right: 20px !important; /* Fixed padding to stabilize scrollbar space */
            }
            .step-2.active .directory-grid, .step-3.active .directory-grid {
                padding-right: 0 !important; /* Remove variable padding */
                margin-right: 20px !important; /* Fixed margin for consistent scrollbar space */
                overflow-y: auto;
    ...
    function generateColorIcons(item) {
        ...
        iconColors.forEach((color, index) => {
            ...
            // Only disable if animation is actively playing (relevant for step 2)
            if (!isStep3) {
            ...
            iconDiv.addEventListener('click', function(e) {
                ...
                updateBackgroundWidth(); // Force recalculation to stabilize layout
            });
        });
        ...
        directoryItems.forEach(item => {
            item.addEventListener('click', function(e) {
                ...
                if (input.checked) {
                    ...
                    // For step 2, hide icons if input is not checked
                    selectedItem = null;
                    this.classList.remove('selected');
                    animateHideIcons();
                }
                updateBackgroundWidth(); // Force recalculation to prevent shifts
            });
        });
    }
    ...
    function init() {
        ...
        // Only call updateBackgroundWidth for step 2 or 3
        if (document.querySelector('.step-2.active, .step-3.active')) {
            updateBackgroundWidth();
            window.showColorIconsForSelected();
        } else {
            // Ensure step 1 has no residual styles
            const rightSection = document.querySelector('.step-1.active .right-section');
            if (rightSection) {
                rightSection.style.width = '';
                rightSection.style.minWidth = '';
        ...
        window.addEventListener('resize', () => {
            if (document.querySelector('.step-2.active, .step-3.active')) {
                ...
                // Reset styles on resize if on step 1
                const rightSection = document.querySelector('.step-1.active .right-section');
                if (rightSection) {
                    rightSection.style.width = '';
                    rightSection.style.minWidth = '';
        ...
        const observer = new MutationObserver(function(mutations) {
            ...
            if (activeStep !== currentActiveStep) {
                ...
                // Only update for step 2 or 3
                if (document.querySelector('.step-2.active, .step-3.active')) {
                    updateBackgroundWidth();
                    updateColorIconsPosition();
                    window.showColorIconsForSelected();
                } else if (document.querySelector('.step-1.active')) {
                    // Reset styles for step 1
                    const rightSection = document.querySelector('.step-1.active .right-section');
                    if (rightSection) {
                        rightSection.style.width = '';
                        rightSection.style.minWidth = '';
