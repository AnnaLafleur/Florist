document.addEventListener('DOMContentLoaded', function() {
    ...
    class Packer {
        ...
        estimateContainerRadius(n) {
            if (n === 1) return unitRadius * 0.6; // Радиус внешнего круга для n=1
    ...
    function createRegionPaths(packer, regions, svg, gridSize, border, zoom, margin) {
        ...
        validRegions.forEach((region, regionIndex) => {
            ...
            path.addEventListener('mouseenter', function(e) {
                ...
                if (!greenRadio || !greenRadio.checked) return;
    ...
    function drawFlowers(n) {
        ...
        if (n === 1) {
            // Для n=1 используем центр внутреннего круга
            const innerCircle = packer.list[0];
            const centerX = (innerCircle.c.x + w / 2) * zoom + margin;
        ...
        // Отрисовка свободных областей для n !== 1
        if (n !== 1) {
            try {
