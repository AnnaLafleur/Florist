document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateFreeAreas(packer, width, height, margin, zoom) {
        const gridSize = 5; // Увеличен размер ячейки сетки для большей точности
        const circles = packer.list.map(circle => ({
            x: margin + circle.c.x * zoom,
            y: margin + circle.c.y * zoom,
            r: circle.r * zoom + Math.min(0.3, zoom * 0.01) // Адаптивный буфер
        }));
        ...
        function isOutsideCircles(x, y) {
            const minDistanceThreshold = gridSize * 0.3; // Уменьшен для сохранения точек у углов
        ...
        function bfs(startX, startY) {
            ...
            while (queue.length > 0) {
                ...
                if (isOutsideCircles(cellCenterX, cellCenterY)) {
                    ...
                    // Добавляем соседей (включаем диагонали для большей связности)
    ...
    function doRegionsIntersect(hull1, hull2) {
        // Проверка пересечения двух выпуклых многоугольников методом разделяющей оси
        ...
        function getAxes(hull) {
            ...
            for (let i = 0; i < hull.length; i++) {
                ...
                const normal = [-edge[1], edge[0]]; // Перпендикуляр
        ...
        const buffer = 1; // Буфер для предотвращения близких областей
        ...
        return true; // Пересечение или слишком близкое расположение
    }

    function createRegionPaths(regions, svg, gridSize, border) {
        ...
        // Применяем clipPath к группе регионов
        regionsGroup.setAttribute("clip-path", "url(#border-clip)");

        // Фильтруем области, чтобы избежать пересечений и оставить только крупные области
        const validRegions = [];
        ...
        // Порог площади (уменьшен до 0.06 от общей площади прямоугольника)
        const totalArea = border.width * border.height;
        const minAreaThreshold = 0.06 * totalArea;

        console.log(`Total Area: ${totalArea}, Min Area Threshold: ${minAreaThreshold}`);
        ...
        regions.forEach((region, index) => {
            ...
            // Добавляем угловые точки прямоугольника, если точки оболочки близки к ним
            const cornerThreshold = gridSize; // Порог для добавления углов
            corners.forEach(corner => {
                const [cx, cy] = corner;
                if (hull.some(p => Math.sqrt((p[0] - cx) ** 2 + (p[1] - cy) ** 2) < cornerThreshold)) {
                    // Проверяем, что уголовая точка не слишком близко к кругам
            ...
            // Перестраиваем выпуклую оболочку после добавления угловых точек
            ...
            // Вычисляем размеры выпуклой оболочки
            ...
            console.log(`Region ${index}: Points=${region.points.length}, Width=${width}, Height=${height}, ApproxArea=${approxArea}, Pass=${approxArea >= minAreaThreshold}`);

            // Пропускаем области, которые меньше порога
            if (approxArea < minAreaThreshold) {
                console.log(`Region ${index} skipped: area too small (${approxArea} < ${minAreaThreshold})`);
                return;
            }

            // Проверяем пересечение с уже добавленными оболочками
        ...
        validRegions.forEach(region => {
            ...
            // Создаём путь для области
            const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
            // Используем кривые Безье для сглаживания
            let pathData = `M ${hull[0][0]} ${hull[0][1]}`;
            const controlPointDist = 0.1 * gridSize; // Уменьшено для меньшего сглаживания
            ...
            // Вычисляем центр масс (центроид) области
            let cx = 0, cy = 0;
            const n = hull.length;
            let signedArea = 0;
            for (let i = 0; i < n; i++) {
                const x0 = hull[i][0];
                const y0 = hull[i][1];
                const x1 = hull[(i + 1) % n][0];
                const y1 = hull[(i + 1) % n][1];
                const a = x0 * y1 - x1 * y0;
                signedArea += a;
                cx += (x0 + x1) * a;
                cy += (y0 + y1) * a;
            ...
            // Определяем размер плюса пропорционально размеру области
            const xCoords = hull.map(p => p[0]);
            const yCoords = hull.map(p => p[1]);
            const width = Math.max(...xCoords) - Math.min(...xCoords);
            const height = Math.max(...yCoords) - Math.min(...yCoords);
            const minDimension = Math.min(width, height);
            const basePlusSize = gridSize * 2; // Минимальный размер, аналогичный кругам
            const plusSize = Math.max(basePlusSize, minDimension * 0.2); // Пропорционально области, но не меньше кругов

            // Создаём горизонтальную линию плюса
            ...
            // Создаём вертикальную линию плюса
    ...
    function drawFlowers(n) {
        ...
        const border = {
            x: margin + (w * zoom - w * zoom * (n === 1 ? 0.7 : 1)) / 2,
            y: margin + (h * zoom - h * zoom * (n === 1 ? 0.7 : 1)) / 2,
            width: w * zoom * (n === 1 ? 0.7 : 1),
            height: h * zoom * (n === 1 ? 0.7 : 1)
        };
        ...
        svg.setAttribute('viewBox', `0 ${-offsetY} ${w * zoom + margin * 2} ${viewBoxHeight}`);
        ...
        // Вычисляем и рисуем свободные области только если n не равно 1 или 3
        if (n !== 1 && n !== 3) {
            const { regions, gridSize } = calculateFreeAreas(packer, w, h, margin, zoom);
            createRegionPaths(regions, svg, gridSize, border);
        }

        // Рисуем круги поверх областей
        packer.list.forEach(circle => {
            const cx = margin + circle.c.x * zoom;
            const cy = margin + circle.c.y * zoom;
            const radius = circle.r * zoom * (n === 1 ? 0.7 : 1);
