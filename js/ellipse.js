document.addEventListener('DOMContentLoaded', function() {
    ...
    svgEllipse.style.width = '100%'; // Полная ширина экрана
    svgEllipse.style.height = 'auto';

    // Параметры эллипса
    const unitRadius = 30; // Согласовали с circle.js
    const perspectiveScale = 0.7;
    const margin = 20;
    const scaleFactor = 5; // Увеличение на 500%
    ...
    function drawEllipse() {
        ...
        // Динамический расчет zoom только по высоте
        const maxHeight = window.innerHeight * 0.8 - margin * 2;
        const zoom = maxHeight / (h * perspectiveScale);

        // Получаем старый viewBox из flowers-svg
        ...
        const bgYOffset = 100 * scaleFactor;

        // Центр эллипса: фиксированный отступ в 40% ширины окна слева + половина ширины группы
        const groupWidth = w * zoom * scaleFactor;
        const ellipseCenterX = -extraLeftPadding - margin + 0.4 * window.innerWidth + (groupWidth / 2);
        const ellipseCenterY = (viewBoxHeight / 2 - viewBoxYOffset - extraTopPadding) - viewBoxHeight * 0.02 - 15 - 120 * scaleFactor;

        // Фоновое изображение (paper2)
        ...
        paper2.setAttribute('x', (ellipseCenterX - bgWidth / 2 - 25 * scaleFactor).toString());
        paper2.setAttribute('y', (ellipseCenterY - bgHeight / 2 + bgYOffset + 70 * scaleFactor).toString());
        backgroundGroup.appendChild(paper2);

        // Верхнее изображение (paper1)
        ...
        paper1.setAttribute('width', (bgWidth * 1.05).toString());
        paper1.setAttribute('height', bgHeight.toString());
        paper1.setAttribute('x', (ellipseCenterX - bgWidth / 2 - 5 * scaleFactor).toString());
        paper1.setAttribute('y', (ellipseCenterY - bgHeight / 2 + bgYOffset + 100 * scaleFactor).toString());
        foregroundGroup.appendChild(paper1);

        // Сортируем цветы по cy
        ...
        sortedFlowerData.forEach(flower => {
            ...
            // Новые абсолютные координаты с учетом масштаба
    ...
    // Создаем контейнер и кнопку сохранения
    ...
    // Наблюдатель за изменениями классов этапов
