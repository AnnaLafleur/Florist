document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateItems(grid) {
        if (grid.style.display === 'none') return; // Skip animation for hidden grid
    ...
    function toggleGrids(showTexture) {
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => { isAnimating = false; }, 1000); // Reset after animation
        });
    }
    ...
    printGrid.addEventListener('click', (event) => {
        ...
        if (checkbox.checked) {
            ...
            setTimeout(() => {
                isAnimating = false;
            }, 1450); // Account for animation duration
        }
