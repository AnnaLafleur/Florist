document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentPaperButton = 'paper1'; // Track the currently active paper button
    const scrollPositions = { paper1: 0, paper2: 0 }; // Store scroll positions for paper1 and paper2
    ...
    function updateBackgroundWidth() {
        ...
        const columnGap = 8; // As requested
    ...
    function initializeGrids() {
        requestAnimationFrame(() => {
            const commonStylesStep3 = {
                ...
                columnGap: '15px',
                width: '100%',
                paddingBottom: '20px', // Updated to match creator.css
                boxSizing: 'border-box',
                height: 'calc(100% - 125px)', // Updated to match creator.css
                overflowY: 'auto',
    ...
    function animateItems(grid) {
        ...
        items.forEach(item => {
            item.style.transform = 'rotateY(180deg)'; // Start at 180 degrees
        ...
        return new Promise(resolve => {
            requestAnimationFrame(() => {
                currentGridAnimation = anime({
                    targets: items,
                    rotateY: [180, 0], // Animate from 180 to 0 degrees
                    duration: 1000,
    ...
    function toggleGrids(showTexture, isInitial = false) {
        ...
        if (selectedShape !== 'form-circle') {
            ...
            if (!isInitial) {
                ...
                printItems.forEach(item => {
                    item.style.transform = 'rotateY(180deg)'; // Updated to match animation start
                    item.offsetHeight;
                });
                requestAnimationFrame(() => {
        ...
        // Interrupt ongoing animations for both grids
        if (currentGridAnimation) {
            currentGridAnimation.pause();
            isAnimating = false;
            currentGridAnimation = null;
    ...
    function createPaperButton(iconSrc) {
        ...
        button.addEventListener('click', function(e) {
            ...
            // Interrupt ongoing grid animation
            if (currentGridAnimation) {
                currentGridAnimation.pause();
                isAnimating = false;
                currentGridAnimation = null;
            ...
            // Save current scroll position and update button state
            ...
            // Restore scroll position and start new animation
            textureGrid.scrollTop = scrollPositions[currentPaperButton];
            requestAnimationFrame(() => {
                textureGrid.scrollTo({ top: 0, behavior: 'smooth' });
    ...
    // Стили с ровной обводкой из центра
