document.addEventListener('DOMContentLoaded', () => {
    ...
    // Добавленный код для снятия выделения при выходе из этапа
    function clearSelectionOnStageChange() {
        ...
        if (!isSecondStage) {
            // Снимаем выделение со всех радио-кнопок цветов
        ...
        if (!isThirdStage) {
            // Снимаем выделение со всех радио-кнопок текстур
    ...
    // Наблюдаем за изменениями в DOM для отслеживания смены этапов
    ...
    // Начинаем наблюдение за элементами этапов
    document.querySelectorAll('.step-1, .step-2, .step-3').forEach(element => {
        stageObserver.observe(element, { attributes: true });
    });

    // Вызываем сразу на случай, если страница загрузилась не на втором этапе
    clearSelectionOnStageChange();
});