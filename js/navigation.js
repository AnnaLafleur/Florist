document.addEventListener('DOMContentLoaded', function() {
    ...
    function showStep(stepIndex) {
        ...
        // Управляем видимостью color-icon-container через класс на body
        if (stepIndex === 1) {
            document.body.classList.add('step-2-visible');
        } else {
            document.body.classList.remove('step-2-visible');
        }
    ...
    document.querySelectorAll('.next-btn-short').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            window.hideColorIconsImmediately(); // Скрываем контейнер с иконками сразу
            hideContent();
